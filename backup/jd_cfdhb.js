...
function cashOut() {
    return new Promise(async (resolve) => {
        $.get(
            taskUrl(
                `user/ExchangePrize`,
                `ddwPaperMoney=100000&&strPoolName=jxcfd2_exchange_hb_2021&strPgUUNum=${token['farm_jstoken']}&strPgtimestamp=${token['timestamp']}&strPhoneID=${token['phoneid']}`
            ),
            async (err, resp, data) => {
                try {
                    if (err) {
...
function taskUrl(function_path, body) {
    let url =  `${JD_API_HOST}jxbfd/${function_path}?strZone=jxbfd&bizCode=jxbfd&source=jxbfd&dwEnv=7&_cfd_t=${Date.now()}&ptag=138631.26.55&dwType=3&dwLvl=9&${body}&_stk=_cfd_t%2CbizCode%2CddwPaperMoney%2CdwEnv%2CdwLvl%2CdwType%2Cptag%2Csource%2CstrPoolName%2CstrZone&_ste=1`;
